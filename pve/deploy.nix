...
let
  ...
  tsFwd =
    if (cfg ? tailscaleForward && cfg.tailscaleForward) then
      ''
  ...
  applySh = pkgs.writeShellScript "${host}-apply" (
...
in
pkgs.writeShellScriptBin "${host}-deploy" (
  builtins.replaceStrings
    [
      "@FILES@"
      "@APPLY@"
    ...
    [
      ...
      tsPush
      luckyPush
      (builtins.readFile ./self-deploy.sh)
    ]

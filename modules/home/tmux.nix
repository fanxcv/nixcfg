# tmux + gpakosz 配置模块（从 _common_/tmux.nix 迁入，加 softwares.tmux.enable 门控）
# 对应 docker/ide/ubuntu/Dockerfile 里的 .tmux clone + 定制
...
}:
{
  ...
  config = lib.mkIf config.softwares.tmux.enable {

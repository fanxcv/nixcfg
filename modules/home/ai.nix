...

{
  ...
  tools,
  ...
}: # GitHub 加速前缀/开关从集中配置 tools/config.nix 读（tools.githubUrl）
let
  ...
  os =
    if pkgs.stdenv.hostPlatform.isLinux then
      ...
      throw "unsupported os: ${pkgs.stdenv.hostPlatform.system}";
  arch =
    if pkgs.stdenv.hostPlatform.isAarch64 then
      ...
      throw "unsupported arch: ${pkgs.stdenv.hostPlatform.system}";

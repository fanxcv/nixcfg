# ide-si 容器 mise 组件声明（wanted.yaml 的 linux.ubuntu.ide-si.mise 维护）
#   组件清单从容器实测导出（mise ls --installed），纳入 nix 管理：
#     bun/go/java(zulu-8)/maven/node/python/uv（lenovo 另有 pipx、java oracle-17，见 ../ide-lenovo/mise.nix）

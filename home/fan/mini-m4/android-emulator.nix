...
# AVD 定义对齐实机现状：Pixel_Fold(android-37.0 ps16k) / Pixel_Tablet(android-35 tablet)
...
{ lib, ... }:
{
  home.activation.setupAndroidEmulator = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [ -x "$ANDROID_HOME/emulator/emulator" ] \
      && [ -d "$ANDROID_HOME/system-images/android-35/google_apis_playstore_tablet/arm64-v8a" ] \
    ...
    create_avd Pixel_Tablet "system-images;android-35;google_apis_playstore_tablet;arm64-v8a" pixel_tablet
  '';
}
